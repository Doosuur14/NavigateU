...
class RegistrationScreenViewController<ViewModel: RegistrationViewModelProtocol>: UIViewController {
    ...
    @objc func registerButtonTapped() {
        ...
        viewModel.register(firstName: firstName, lastName: lastName, email: email, password: password, countryOfOrigin: countryOfOrigin, cityOfResidence: cityOfResidence)
    }

    private func configureIO() {
        viewModel.stateDidChangeForReg.sink { [weak self]  in
            self?.render()
        }
